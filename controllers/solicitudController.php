...
class solicitudController
{
    ...
    public function crearPost(): void
    {
        ...
        flash_set(
            $ok ? 'success' : 'error',
            $ok ? 'Solicitud creada correctamente.' : 'No se pudo guardar la solicitud.'
        );
    ...
    public function gestionJefePost(int $id): void
    {
        ...
        $ok = $accion === 'aprobar'
            ...
            : $this->model->rechazarJefe($id, $user['cedula'], $obs);

        if ($ok && $solicitud) {
            $notificaciones = new notificacionService();

            if ($accion === 'aprobar') {
                $notificaciones->notificarAprobacionJefe(
                    $id,
                    (string)$solicitud['NIT_EMPLEADO'],
                    (string)$user['cedula'],
                    (string)$solicitud['TIPO_SOLICITUD']
                );
            } else {
                $notificaciones->notificarRechazoJefe(
                    $id,
                    (string)$solicitud['NIT_EMPLEADO'],
                    (string)$user['cedula'],
                    (string)$solicitud['TIPO_SOLICITUD'],
        ...
        flash_set(
            $ok ? 'success' : 'error',
            $ok ? 'Gestion de jefe guardada.' : 'No se pudo gestionar la solicitud.'
        );
    ...
    public function gestionRrhhPost(int $id): void
    {
        ...
        $ok = $accion === 'aprobar'
            ...
            : $this->model->rechazarRRHH($id, $user['cedula'], $obs);

        if ($ok && $solicitud) {
            $notificaciones = new notificacionService();

            if ($accion === 'aprobar') {
                $notificaciones->notificarAprobacionRRHH(
                    $id,
                    (string)$solicitud['NIT_EMPLEADO'],
                ...
                $notificaciones->notificarRechazoRRHH(
                    $id,
                    (string)$solicitud['NIT_EMPLEADO'],
                    (string)$solicitud['NIT_JEFE'],
                    (string)$solicitud['TIPO_SOLICITUD'],
        ...
        flash_set(
            $ok ? 'success' : 'error',
            $ok ? 'Gestion de RRHH guardada.' : 'No se pudo gestionar la solicitud.'
        );
    ...
    public function eliminar(int $id): void
    {
        ...
        flash_set(
            $ok ? 'success' : 'error',
            $ok ? 'Solicitud eliminada.' : 'No se pudo eliminar.'
        );
    ...
    private function jefesDisponibles(empleadoModel $empleadoModel): array
    {
        ...
        uasort(
            $unicos,
            static fn($a, $b) => strcmp((string)($a['NOMBRE_COMPLETO'] ?? ''), (string)($b['NOMBRE_COMPLETO'] ?? ''))
        );
